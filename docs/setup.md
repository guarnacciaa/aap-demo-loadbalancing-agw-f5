...
- **Node naming — IP-based vs. hostname-based**: BIG-IP's pool member is backed by a `node` object whose `name` is independent of its `address`; many real environments name nodes after the target VM's hostname (for example `SECOPSTEST02UAT:80`) rather than its IP (`10.233.166.73:80`). `f5networks.f5_modules.bigip_pool_member`'s own `name` parameter is documented as the "name of the node to create **or re-use**" — when it is left unset, the module derives an expected name from `address` alone and, on a hostname-named pool, never matches the real node. The module then concludes the member does not exist and attempts to **create** a brand-new, IP-named one instead of modifying the existing one — an operation many restricted BIG-IP automation accounts correctly reject (`Access Denied: user (...) does not have create access to object (pool_member)`), even though the very same account can toggle the existing member's state from the GUI without any error. `tasks/f5_discover_pool_from_ip.yml` parses the real node name from the member's own `name` field (the same way it already parses the port) and every `LB - *` scenario template threads it through as `bigip_pool_member`'s `name:` parameter — this works transparently for both naming conventions, since an IP-named member's derived node name is identical to its address.
- `f5_pool_name`/`f5_partition`/`f5_pool_member_port`/`f5_node_name` are `[LAB/DEV ONLY]`: the only consumers are `Setup - F5 pool member` / `Teardown - F5 pool member`, which need an explicit pool identity to create (or remove) the *very first* membership — before that membership exists, there is nothing for discovery to find. They are never read by any `LB - *` scenario template (which discovers the node name automatically per-VM, as described above), though `f5_pool_name` (and, if the target pool is hostname-named, `f5_node_name`) can still be passed as an explicit override (`-e f5_pool_name=... -e f5_node_name=...`) at launch to bypass discovery and restrict an action to one specific pool. `f5_pool_member_port` specifically is a further **optional override** on top of that: both `Setup`/`Teardown - F5 pool member` fall back to `80` (`| default(80)` in the tasks themselves) when it is left commented out in `demo_variables.yml.example` — set it explicitly only if the very first member must be registered on a different port (for example an HTTPS-only backend on `443`).
- Required RBAC: the F5 API credential needs read access to **every partition** the target node's pools might live in, plus pool-member write access to modify them. A credential scoped to a single partition limits discovery (and therefore drain/reconnect) to what it can see in that partition. Note that the "does not have create access" symptom described above under node naming looks identical to an RBAC gap at first glance, but is actually a naming mismatch — rule it out first using the guidance above before requesting a broader BIG-IP role for the automation account.
