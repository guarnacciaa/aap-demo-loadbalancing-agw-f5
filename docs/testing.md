...
- AGW drain/disconnect is not yet confirmed working end-to-end. Two more fixes landed 2026-07-28 (`azure_rm_networkinterface`'s `virtual_network` resource-group handling; the `drain_poll_interval`/`lb_backend_type`/`target_vm_hostname` self-referencing var recursion) — re-run `LB - Drain and disconnect VM` next session and update this log with the actual result, including whether the drain-wait pause now completes without error.
